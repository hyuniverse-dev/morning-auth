...
data class ReqOperatorDTO(
    ...
    @Schema(
        description = "접근권한",
        nullable = false,
        example = "5c2563d57a2211eeb1d10242ac130002"
    )
    @field: NotBlank(message = "{roleCode.not.blank}")
    var roleCode: String?,


    @Schema(description = "관리자명", nullable = false, example = "옥순")
    ...
    @JsonIgnore
    private val managerCode: String = generateUUID()
    ...
    init {
        ...
        this.nationalityCode = nationalityCode?.trim()
        this.address = address?.trim()
        if (password?.trim()?.isNotBlank() == true) {
